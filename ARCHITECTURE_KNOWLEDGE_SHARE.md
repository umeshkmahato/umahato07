# Architecture Knowledge Sharing (Phase 1 + Phase 2 + Phase 3)
...
### Agentic GenAI (abstracted)
- Added `AgentOrchestrator` to convert user query + context into a structured execution plan.
- Added `LanguageModelClient` interface with current implementation `MockLanguageModelClient`.
- Mock client returns plan JSON (reasoning + tool steps), enabling easy swap to a real provider later via env config:
  - `GENAI_PROVIDER`
  - `GENAI_MODEL`
  - `GENAI_API_KEY`
- `AgentQueryResponse` now returns the executed plan (`reasoning`, `steps`) for transparency.

### Agent tool plan + execution model
- Supported tool steps:
  - `FETCH_USER`
  - `FETCH_SCORE`
  - `UPDATE_SESSION`
  - `PUBLISH_EVENT`
- Agent service executes tool steps in order, with fallback behavior if model output is malformed.

### Redis + Kafka refinement
- Redis session context key `agent-session:{sessionId}` is read before plan generation and updated after query handling.
- Kafka producer publishes `UserAgentQueried` event.
- Added Kafka consumer (`UserAgentEventConsumer`) to log consumed events from `user-agent-events`.
...
- Added Dockerfiles for:
  ...
  - `agent-service`
  - `service-discovery`
- Added `docker-compose.yml` bringing up:
  - service-discovery, api-gateway, user-service, agent-service
  - Postgres, Redis, Zookeeper, Kafka
  - profile-score mock service (returns deterministic score)

### Dev Container
- Added `.devcontainer/devcontainer.json` with:
  - Java 21 devcontainer image
  - Docker-outside-of-Docker feature
  - post-create Maven build command

### Test coverage added
- `UserControllerCachingIntegrationTest` validates `/users/{id}` cache behavior (DB hit once, cache hit next).
- `AgentQueryOrchestrationTest` validates `/agent/query` orchestration flow with mocked collaborators.
